    ...
    while true
        ...
        if type(msg) = "roPosterScreenEvent" then
            ...
            if msg.isListFocused() then
                ...
                m.curShow = displayShowDetailScreen(category, m.curShow, m.Shows)
                screen.SetFocusedListItem(m.curShow)
...
Function showSearchResultPosterScreen(screen As Object, category As Object, searchQuery As String) As Integer

    if validateParam(screen, "roPosterScreen", "showSearchResultPosterScreen") = false return -1
    if validateParam(category, "roAssociativeArray", "showSearchResultPosterScreen") = false return -1

    m.curCategory = 0
    m.curShow     = 0

    searchFeed = newShowFeed()
    for each item in m.Shows
        if Instr(1, LCase(item.Title), LCase(searchQuery)) > 0 then
            searchFeed.Push(item)
        else if Instr(1, LCase(item.Description), LCase(searchQuery)) > 0 then
            searchFeed.Push(item)
        endif
    end for

    screen.SetContentList(searchFeed)
    screen.Show()
    ...
    while true
        ...
        if type(msg) = "roPosterScreenEvent" then
            ...
            if msg.isListFocused() then
                ...
                m.curShow = msg.GetIndex()
                'print "list item selected | current show = "; m.curShow
                m.curShow = displayShowDetailScreen(category, m.curShow, searchFeed)
                screen.SetFocusedListItem(m.curShow)
                'print "list item updated  | new show = "; m.curShow
            else if msg.isScreenClosed() then
                return -1
...
'**********************************************************
Function displayShowDetailScreen(category as Object, showIndex as Integer, shows as Object) As Integer
    ...
    showIndex = showDetailScreen(screen, shows, showIndex, false)
